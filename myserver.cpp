...
MyServer::MyServer(QObject *parent) : QObject(parent)
{
    ...
    AppConfig config;
    m_printer1 = config.printer1();
    m_printer2 = config.printer2();
    m_printer3 = config.printer3();
    m_printer4 = config.printer4();
    m_printer5 = config.printer5();

    qDebug() << "printer1:"<<m_printer1;
    qDebug() << "printer2:"<<m_printer2;
    qDebug() << "printer3:"<< m_printer3;
    qDebug() << "printer4:"<< m_printer4;
    qDebug() << "printer5:"<< m_printer5;
...
void MyServer::ReceiveDataProcess()
{
    ...
    int count = tempSocket->availableMsg();

    if(count>0)
    {
        for(int i=0; i<tempSocket->msg_list.count(); i++)
        {
            QString recvstr = tempSocket->msg_list.at(i);
    ...
    {
        qDebug() << "ReceiveDataProcess: search frame failed!\n";
    }
}
...
//    score_info.suggestion = "太棒了，不要骄傲哦！";
    ...
    {
        if(printer_info_list.at(i).printerName() == m_printer1)
        {
            printer_info_array[PRINTER_ONE] = printer_info_list.at(i);
        }
        else if(printer_info_list.at(i).printerName() == m_printer2)
        {
            printer_info_array[PRINTER_TWO] = printer_info_list.at(i);
        }
        else if(printer_info_list.at(i).printerName() == m_printer3)
        {
            printer_info_array[PRINTER_THREE] = printer_info_list.at(i);
        }
        else if(printer_info_list.at(i).printerName() == m_printer4)
        {
            printer_info_array[PRINTER_FOUR] = printer_info_list.at(i);
        }
        else if(printer_info_list.at(i).printerName() == m_printer5)
        {
            printer_info_array[PRINTER_FIVE] = printer_info_list.at(i);
        }
    }
    ...
    case COM4:
        ...
        break;
    case COM5:
            do_the_print(printer_info_array[PRINTER_FIVE],edit,score_info.name);
        break;
    default:
...
void MyServer::do_the_print(QPrinterInfo info, QTextEdit *edit, QString name)
{
    ...
    QString path = QString("D:\\ScorePrint\\%1 - %2.pdf").arg(current_date).arg(name);
//    p.setOutputFileName(path);
    td->print(&p);
    qDebug() << "================== do the print end......\n";
}
